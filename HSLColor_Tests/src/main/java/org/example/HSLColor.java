...
/**
 * Your HSLColor class.
 * I've added a package declaration and made the class public
 * so the test class (in the same package) can access it.
 */
public class HSLColor {
    // Note: Constants are usually static final and named in UPPER_SNAKE_CASE
    private final static int HSLMAX = 255;
    private final static int RGBMAX = 255;
    private final static int UNDEFINED = 170; // This seems arbitrary, often -1 or Integer.MIN_VALUE is used
    ...
    // Added a constructor for easier testing
    public HSLColor() {
        // Default constructor
    }
    ...
    public void initHSLbyRGB(int R, int G, int B) {
        ...
        if (cMax == cMin) {
            ...
            // Ensure hue is within range, as calculations might slightly exceed
            if (pHue > HSLMAX) {
                pHue = HSLMAX;
            }
        }
